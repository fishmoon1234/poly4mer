...
def main(config):
    ...
    checkpoint = torch.load(f"{base_dir}/checkpoint/decoder_predictor/model_params{count_params(lang_model) + count_params(decoder)}_last_epoch.ckpt", weights_only=True)
    ...
    [os.makedirs(f"{base_dir}/results/{subdir}/{save_file}", exist_ok=True) for subdir in base_paths]
    

    with open(f"{base_dir}/results/optimized_embeddings/{save_file}/optimized_embeddings_{smiles_idx}_lr_{config.lr_start:.4f}.txt", "w") as f:
        for embd in save_embeddings:
            np.savetxt(f, embd.cpu().numpy(), fmt="%.6f")
    
    with open(f"{base_dir}/results/optimized_smiles/{save_file}/optimized_smiles_{smiles_idx}_lr_{config.lr_start:.4f}.txt", "w") as f:
        for ep, smiles in save_smiles:
            f.write(f"Epoch {ep}: {smiles}\n")
    
    with open(f"{base_dir}/results/optimized_smiles_canonicalized/{save_file}/optimized_smiles_canonicalized_{smiles_idx}_lr_{config.lr_start:.4f}.txt", "w") as f:
        for ep, smiles in save_canonicalized_smiles:
            f.write(f"Epoch {ep}: {smiles}\n")

    with open(f"{base_dir}/results/log_optimized_prop/{save_file}/log_optimized_props_{smiles_idx}_lr_{config.lr_start:.4f}.txt", "w") as f:
