...
class LangLayer1(nn.Module):
    def __init__(self, n_embd, n_vocab):
        super().__init__()
        self.is_cuda_available = torch.cuda.is_available()
        self.embed1 = nn.Linear(n_embd, n_embd * 4)
        self.ln_f1 = nn.LayerNorm(n_embd * 4)
        self.embed2 = nn.Linear(n_embd * 4, n_embd * 4)
        self.ln_f2 = nn.LayerNorm(n_embd * 4)
        self.head = nn.Linear(n_embd * 4, n_vocab, bias=False)
    def forward(self, tensor):
        if self.is_cuda_available:
            self.embed1.cuda()
            self.ln_f1.cuda()
            ...
            self.head.cuda()
            tensor = tensor.cuda()
        tensor = self.embed1(tensor)
        tensor = F.gelu(tensor)
        tensor = self.ln_f1(tensor)
        tensor = self.embed2(tensor)
        tensor = F.gelu(tensor)
        tensor = self.ln_f2(tensor)
        tensor = self.head(tensor)
        return tensor
...
class MoLDecoder(nn.Module):
    ...
    def __init__(self, n_vocab, max_len, n_embd, n_gpu=None):
        ...
        self.lang_model1 = LangLayer1(n_vocab, 2)
        # self.lang_model1 = LangLayer1(n_embd, 2)
